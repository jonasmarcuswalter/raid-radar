...
function renderNextCards() {
  ...
  const nextFood =
    ...
    nextPoi((poi) => isFoodPoi(poi) && poi.route_km >= km && poi.source_status !== "unverified") ||
...
async function downloadMapPack() {
  ...
  if (!configured.url) {
    saveMapPackStatus({
      ...
      note: "PMTiles noch nicht hinterlegt. Das lokale Route-Korridor-Pack bleibt als Offline-Basemap nutzbar.",
    });
    renderOfflinePanel();
